...
const SectionTab = ({ icon: Icon, label, active, onClick }) => (
  <button
    ...
    className={`flex items-center gap-3 px-5 py-3 rounded-xl text-sm font-bold transition-all ${
      active ? 'bg-accent-gold text-bg-primary shadow-[0_0_15px_rgba(212,175,55,0.3)]' : 'text-text-secondary hover:text-white hover:bg-white/5'
    }`}
  >
    <Icon size={18} />
    {label}
...
const CMS = () => {
  ...
  const inputClass = "w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 font-bold text-sm focus:border-accent-gold outline-none transition-colors placeholder:text-text-secondary"

  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-3xl font-black mb-2">إدارة <span className="text-gold-gradient">الموقع</span></h1>
        <p className="text-text-secondary font-medium">تحكم في محتوى موقعك العام مباشرةً بدون مبرمج.</p>
      </header>

      {/* Live Preview Badge */}
      <div className="flex items-center gap-3 px-5 py-3 glass-panel rounded-xl border-accent-gold/20 border w-fit">
        ...
        {activeTab === 'hero' && (
          <motion.div key="hero" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
            <div className="glass-panel rounded-2xl p-8 space-y-6">
              ...
              <div>
                <label className="text-xs font-black text-text-secondary uppercase tracking-widest mb-2 block">العنوان الفرعي (Subtitle)</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-black text-text-secondary uppercase tracking-widest mb-2 block">الكلمة الذهبية (Highlight)</label>
                ...
                <div>
                  <label className="text-xs font-black text-text-secondary uppercase tracking-widest mb-2 block">باقي العنوان</label>
              ...
              <div>
                <label className="text-xs font-black text-text-secondary uppercase tracking-widest mb-2 block">وصف الشركة</label>
              ...
              {/* Live Preview */}
              <div className="p-6 rounded-xl bg-bg-primary border border-white/5">
                <p className="text-xs font-black text-text-secondary uppercase tracking-widest mb-4">معاينة مباشرة</p>
                ...
                <p className="text-sm text-text-secondary font-medium">{heroForm.description}</p>
              </div>
          ...
          <motion.div key="portfolio" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {webContent.portfolio.map((item) => (
                <div key={item.id} className="glass-panel rounded-xl overflow-hidden border border-white/5 group">
                  ...
                  <div className="p-4">
                    <p className="text-xs text-text-secondary font-medium">{item.description}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <AnimatePresence>
              {showAddPortfolio && (
                ...
                >
                  ...
                  >
                    <div className="p-6 border-b border-white/5 flex justify-between items-center bg-bg-secondary">
                      <h3 className="font-black text-lg">إضافة مشروع لمعرض الأعمال</h3>
                      <button onClick={() => setShowAddPortfolio(false)} className="p-2 text-text-secondary hover:text-red-500 rounded-lg transition-colors"><X size={20} /></button>
                    </div>
                    <form onSubmit={handleAddPortfolio} className="p-6 space-y-4 bg-bg-primary">
                      <div>
                        <label className="text-xs font-black text-text-secondary mb-1 block">اسم المشروع *</label>
                      ...
                      <div>
                        <label className="text-xs font-black text-text-secondary mb-1 block">التصنيف</label>
                      ...
                      <div>
                        <label className="text-xs font-black text-text-secondary mb-1 block flex items-center gap-1"><Image size={12} /> رابط الصورة (URL) *</label>
                      ...
                      <div>
                        <label className="text-xs font-black text-text-secondary mb-1 block">وصف المشروع</label>
                      ...
                      <div className="flex gap-3 pt-2">
                        ...
                        <button type="button" onClick={() => setShowAddPortfolio(false)} className="px-6 py-3 border border-white/10 rounded-lg font-bold text-text-secondary hover:text-white transition-colors">
                          إلغاء
                        </button>
                      </div>
                    </form>
            ...
            <div className="glass-panel rounded-2xl p-8 space-y-6">
              ...
              <div className="space-y-4">
                {webContent.services.map((service) => (
                  <div key={service.id} className="flex items-center gap-4 p-4 bg-bg-primary rounded-xl border border-white/5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-text-secondary font-medium mt-0.5">{service.desc}</p>
                    </div>
                    <button className="p-2 text-text-secondary hover:text-accent-gold transition-colors"><Pencil size={16} /></button>
                  </div>
                ))}
              </div>
              <p className="text-xs text-text-secondary font-bold text-center py-4 border border-white/5 rounded-xl">
